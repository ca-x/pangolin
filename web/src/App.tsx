...
export default function App() {
  ...
  useEffect(()=>{const branding=bootstrap.data?.branding;if(!branding)return;document.title=branding.branding_name;let link=document.querySelector<HTMLLinkElement>("link[rel='icon']");if(!link){link=document.createElement('link');link.rel='icon';document.head.append(link)}link.href=branding.favicon_url},[bootstrap.data?.branding])
  ...
  return <Suspense fallback={<main className="content"><SkeletonRows/></main>}><Routes><Route path="/login" element={<Navigate to="/" replace/>}/><Route element={<Shell user={state.user!} branding={branding}/>}><Route index element={<OverviewPage/>}/><Route path="channels" element={<ChannelsPage/>}/><Route path="models" element={<ModelsPage/>}/><Route path="access" element={<AccessPage/>}/><Route path="prompts" element={<PromptsPage/>}/><Route path="operations" element={<OperationsPage/>}/><Route path="operations/requests/:id" element={<RequestDetailPage/>}/><Route path="operations/traces/:id" element={<TraceDetailPage/>}/><Route path="playground" element={<PlaygroundPage/>}/><Route path="system" element={<SystemPage/>}/></Route><Route path="*" element={<Navigate to="/" replace/>}/></Routes></Suspense>
}
