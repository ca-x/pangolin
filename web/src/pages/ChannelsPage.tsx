...
import { EnabledPill, Field, SelectField, SkeletonRows } from '../components'
...
export default function ChannelsPage() {
  ...
  return <div><div className="section-tabs" role="tablist" aria-label={t('channels')}>{['channels','credentials','channelPolicies','probes','quotas','presets'].map(value=><button key={value} role="tab" aria-selected={tab===value} onClick={()=>setTab(value)}>{t(value)}</button>)}</div>
    {tab==='channels' && <><ResourcePage resource="channels" title={t('channels')} description={t('channelsDescription')} empty={t('channelEmpty')} createLabel={t('addChannel')} columns={[{key:'name',label:t('name'),render:(value,row)=><span className="provider-cell"><ProviderIcon logoKey={String((row.settings as Record<string,unknown>)?.logo_key||'')} name={String(value)}/><strong>{String(value)}</strong></span>},{key:'kind',label:t('providerType')},{key:'base_url',label:t('baseUrl'),mono:true},{key:'enabled',label:t('status'),render:(value)=><EnabledPill enabled={value}/>}]} fields={[{key:'name',label:t('name'),required:true},{key:'kind',label:t('providerType'),kind:'select',required:true,options:providerKinds()},{key:'base_url',label:t('baseUrl'),required:true,defaultValue:'https://api.openai.com'},{key:'settings',label:t('advancedSettings'),kind:'json',defaultValue:{version:1,tags:[],limits:{},circuit:{failures:5,window_ms:60000,recovery_ms:30000}}},{key:'enabled',label:t('status'),kind:'checkbox'}]}/><BulkToggle/></>}
