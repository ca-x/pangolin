...
import { Field, SelectField, SkeletonRows } from '../components'
...
export default function ChannelsPage() {
  ...
  return <div><div className="section-tabs" role="tablist" aria-label={t('channels')}>{['channels','credentials','channelPolicies','probes','quotas','presets'].map(value=><button key={value} role="tab" aria-selected={tab===value} onClick={()=>setTab(value)}>{t(value)}</button>)}</div>
    {tab==='channels' && <><ResourcePage resource="channels" title={t('channels')} description={t('channelsDescription')} empty={t('channelEmpty')} createLabel={t('addChannel')} columns={[{key:'name',label:t('name'),render:(value,row)=><span className="provider-cell"><ProviderIcon logoKey={String((row.settings as Record<string,unknown>)?.logo_key||'')} name={String(value)}/><strong>{String(value)}</strong></span>},{key:'kind',label:t('providerType')},{key:'base_url',label:t('baseUrl'),mono:true},{key:'enabled',label:t('enabled'),render:(value)=>Number(value)?t('enabled'):t('disabled')}]} fields={[{key:'name',label:t('name'),required:true},{key:'kind',label:t('providerType'),kind:'select',required:true,options:providerKinds()},{key:'base_url',label:t('baseUrl'),required:true,defaultValue:'https://api.openai.com'},{key:'settings',label:t('advancedSettings'),kind:'json',defaultValue:{version:1,tags:[],limits:{},circuit:{failures:5,window_ms:60000,recovery_ms:30000}}},{key:'enabled',label:t('enabled'),kind:'checkbox'}]}/><BulkToggle/></>}
    {tab==='credentials' && <CredentialsPanel/>}
    {tab==='channelPolicies' && <ChannelSettingsPanel/>}
...
function useChannelOptions(){const{project}=useProject();const query=useQuery({queryKey:['channel-options',project.id],queryFn:()=>api<Paged<Document>>(projectOperationPath(project.id,'channels')+'?limit=500')});return {query,options:(query.data?.data||[]).map(row=>({value:String(row.id),label:String(row.name) }))}}
function CredentialsPanel(){const{t}=useTranslation();const{options}=useChannelOptions();return <><ResourcePage resource="credentials" title={t('credentials')} description={t('credentialsDescription')} empty={t('credentialEmpty')} createLabel={t('addCredential')} columns={[{key:'provider_name',label:t('channel')},{key:'suffix',label:t('suffix'),mono:true},{key:'credential_type',label:t('credentialType')},{key:'priority',label:t('priority')},{key:'enabled',label:t('enabled'),render:(value)=>Number(value)?t('enabled'):t('disabled')}]} fields={[{key:'provider_id',label:t('channel'),kind:'select',required:true,options},{key:'credential_type',label:t('credentialType'),defaultValue:'api_key',required:true},{key:'secret',label:t('secret'),kind:'secret',hint:t('secretUpdateHint'),omitWhenBlank:true},{key:'priority',label:t('priority'),kind:'number',defaultValue:100},{key:'enabled',label:t('enabled'),kind:'checkbox'},{key:'settings',label:t('advancedSettings'),kind:'json',defaultValue:{version:1}}]}/><BulkToggle resource="credentials"/></>}
function ChannelSettingsPanel(){const{t}=useTranslation();const{options}=useChannelOptions();return <ResourcePage resource="channel-settings" title={t('channelPolicies')} description={t('channelPoliciesHint')} empty={t('channelPoliciesEmpty')} canDelete={false} columns={[{key:'provider_id',label:t('channel'),render:(value)=>options.find(option=>option.value===value)?.label||String(value)},{key:'retry_statuses',label:t('retryStatuses')},{key:'auto_disable_policy',label:t('autoDisable')}]} fields={[{key:'provider_id',label:t('channel'),kind:'select',required:true,options},{key:'endpoint_mappings',label:t('endpointMappings'),kind:'json',defaultValue:{version:1}},{key:'model_rules',label:t('modelRules'),kind:'json',defaultValue:{version:1}},{key:'parameter_overrides',label:t('parameterOverrides'),kind:'json',defaultValue:{version:1}},{key:'retry_statuses',label:t('retryStatuses'),kind:'json',defaultValue:{version:1,statuses:[408,409,429,500,502,503,504]}},{key:'auto_disable_policy',label:t('autoDisable'),kind:'json',defaultValue:{version:1,enabled:false}},{key:'proxy_settings',label:t('proxySettings'),kind:'json',defaultValue:{version:1}}]}/>}
...
function Diagnostics({quota=false}:{quota?:boolean}) {
  const {t}=useTranslation(); const {project}=useProject();const{options}=useChannelOptions();const[selected,setSelected]=useState(''); const run=useMutation({mutationFn:(provider_id:string)=>api(projectOperationPath(project.id,quota?'quota':'probe'),{method:'POST',body:JSON.stringify({provider_id})}),onSuccess:()=>toast.success(t('jobQueued')),onError:(error:Error)=>toast.error(error.message)})
  const current=selected||options[0]?.value||''
  return <form className="diagnostic-bar" onSubmit={(event)=>{event.preventDefault();run.mutate(current)}}><SelectField label={t('channel')} value={current} onValueChange={setSelected} options={options}/><button className="button button-primary" disabled={!current}>{quota?t('collectQuota'):t('runProbe')}</button></form>
}
