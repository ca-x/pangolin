...
export function ResourcePage({ resource, title, description, empty, columns, fields = [], createLabel, immutable, appendOnly, editDisabled = false, endpoint, itemEndpoint, createMethod = 'POST', updateMethod = 'POST', canDelete = true, rowActions, normalize }: ResourcePageProps) {
  ...
  return <>
    ...
    {query.isError ? <QueryError retry={() => void query.refetch()} /> : query.isLoading ? <SkeletonRows /> : rows.length === 0 ? <EmptyState icon={<Inbox />} title={title} copy={filter ? t('noSearchResults') : empty} action={!filter && !immutable && fields.length ? <button className="button" onClick={(event)=>beginCreate(event.currentTarget)}>{createLabel || t('add')}</button> : undefined} /> : <><div className="table-wrap desktop-resource-table" role="region" aria-label={title} tabIndex={0}><table><thead>{table.getHeaderGroups().map((group) => <tr key={group.id}>{group.headers.map((header) => <th key={header.id}>{flexRender(header.column.columnDef.header, header.getContext())}</th>)}</tr>)}</thead><tbody>{table.getRowModel().rows.map((row) => <tr key={row.id}>{row.getVisibleCells().map((cell) => <td key={cell.id}>{flexRender(cell.column.columnDef.cell, cell.getContext())}</td>)}</tr>)}</tbody></table></div><MobileResources rows={rows} columns={columns} onEdit={!immutable&&!appendOnly&&!editDisabled&&fields.length? (row,trigger)=>{lastTrigger.current=trigger;setEditing(row);setOpen(true)}:undefined} onDelete={!immutable&&!appendOnly&&canDelete?(row)=>confirm(t('deleteConfirm'))&&remove.mutate(row.id):undefined}/></>}
...
function MobileResources({rows,columns,onEdit,onDelete}:{rows:Document[];columns:Array<{key:string;label:string;mono?:boolean;render?:(value:unknown,row:Document)=>ReactNode}>;onEdit?: (row:Document,trigger:HTMLButtonElement)=>void;onDelete?: (row:Document)=>void}) {
  const {t}=useTranslation()
  const primary=(row:Document)=>String(row.name||row.public_name||row.email||row.pattern||row.suffix||row.id)
  const state=(row:Document)=>row.enabled!=null?(Number(row.enabled)?t('enabled'):t('disabled')):displayValue(row.status)
  const visible=columns.filter(column=>column.key!=='id').slice(0,3)
  return <div className="mobile-resource-list">{rows.map(row=><article key={row.id}><header><div><strong>{primary(row)}</strong><button className="copy-id" onClick={()=>void navigator.clipboard?.writeText(String(row.id))}>{t('copyId')}</button></div><span>{state(row)}</span></header><dl>{visible.filter(column=>String(row[column.key]??'')!==primary(row)).map(column=><div key={column.key}><dt>{column.label}</dt><dd className={column.mono?'mono-cell':''}>{column.render?column.render(row[column.key],row):displayValue(row[column.key])}</dd></div>)}</dl>{(onEdit||onDelete)&&<footer>{onEdit&&<button className="button button-quiet" onClick={(event)=>onEdit(row,event.currentTarget)}>{t('edit')}</button>}{onDelete&&<button className="button button-quiet danger" onClick={()=>onDelete(row)}>{t('delete')}</button>}</footer>}</article>)}</div>
}
