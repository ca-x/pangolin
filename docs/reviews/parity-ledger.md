...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| observability | 5 | 15 | 0 | 0 | 0 | 0 | 0 | 20 |
| prompts & playground | 6 | 12 | 1 | 0 | 0 | 0 | 1 | 20 |
| models, routing & pricing | 6 | 18 | 2 | 0 | 0 | 0 | 0 | 26 |
| access control | 2 | 17 | 2 | 0 | 0 | 0 | 0 | 21 |
| system settings & background work | 8 | 17 | 5 | 0 | 0 | 0 | 1 | 31 |
| console-wide UX | 3 | 11 | 1 | 0 | 0 | 0 | 0 | 15 |
| **total** | **30** | **90** | **11** | **0** | **0** | **0** | **2** | **133** |

Row count is unchanged at 133 — the six area reports' own findings. No row was
dropped, merged or added. What changed is the status: **131 rows need no work**
(30 already matched, 90 fixed, 11 refuted), **0 rows need work**, and **2 rows are recorded divergences** whose only
...
| 3 | Trend chart carries requests and errors only | closed | fixed | Summary buckets include tokens and cost; Overview renders totals, table, tooltip and spark without inventing missing measurements. | `features/analytics/components/combined-trend-chart.tsx` (requests, tokens, cost) | Rust summary payload guard plus `trendChart.test.tsx` |
...
| 7 | List columns omit the row's own telemetry | closed | fixed | Request rows show requested→resolved model, input/output/cache token facts, TTFT and streaming state from the list projection. | `features/requests/components/requests-columns.tsx:29-35` | `observabilityWiring.test.tsx` telemetry matrix |
...
| 12 | Trace list omits client trace id, request count, first query | closed | fixed | Trace rows project/render client trace ID and request count; one bounded page query derives the earliest captured user text without N+1 or unbounded body reads. | `features/traces/components/traces-columns.tsx:110-180` | Rust bounded preview contract plus `observabilityWiring.test.tsx` |
...
| 14 | Cost is always USD with a fixed six-decimal scale | closed | fixed | Audited instance settings validate display currency while stored money remains integer micro-USD; shared formatting covers configured currencies at six decimals. | `features/analytics/utils/format-currency.ts:1-8`, `system/data/system.ts` `currencyCode` | EUR/JPY `observabilityTruth.test.tsx` cases |
...
| 20 | Model/provider deletion safety | closed | fixed | Model impact/lifecycle guards and channel dependency preview provide explicit typed all-or-nothing refusal before destructive deletes; foreign IDs remain opaque and mutations are audited. | archive dialog + typed delete confirmation | model lifecycle/delete and B60 preview/bulk-delete cases |
...
| 19 | One restore conflict strategy for the whole artifact | closed | fixed | Project artifact restore accepts a validated default plus per-resource fail/skip/overwrite strategies, preserving secrets and the legacy fail default; the separate instance archive remains an additional capability. | per-class strategies (`RestoreOptionsInput`) | `b51_restore_applies_per_resource_strategies_and_defaults_to_fail`, `SystemPage.test.tsx` |
...
| 22 | No instance-level general settings | closed | fixed | Strict audited instance settings add display currency and an IANA timezone while authoritative money remains integer micro-USD; shared monetary and principal system-date surfaces consume the policy. | `general-settings.tsx:149-224` | instance settings Rust round-trip plus `instanceSettings.test.tsx`/observability formatting cases |
...
| 26 | Outbound proxy presets, and per-webhook timeout/proxy | closed | fixed | Encrypted instance presets are reusable by provider egress, bounded-timeout webhooks and catalog sources; provider resolution pins proxy DNS, direct channel proxy overrides remain supported and catalog fetches default to no proxy. | `proxy-presets-settings.tsx:29-98`, `webhook-settings.tsx:424-522` | B53 webhook/catalog tests plus provider-preset egress contract and `SystemPage.test.tsx` |
