...
| 13 | Invitations are single-use and email-bound | Invite dialog issues a reusable link with expiry presets (1/6/24/168 hours, 0 = never) and `maxUses` 1 or unlimited, no email (`proejct-users/components/users-invite-dialog.tsx:20-23`) | Console sends `expires_in_seconds` with no max-uses (`AccessPage.tsx:324`); `InvitationInput` has no max-uses (`src/access.rs:326-330`), lifetime is bounded to 60s–30d (`src/access.rs:1112`) and acceptance is single-use and bound to the invited email (`src/access.rs:1203-1218`) | feature-build | Add max-uses if reusable links are wanted; Pangolin's 30-day bound and email binding are deliberate, not gaps |
